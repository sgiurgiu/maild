...
session::session(boost::asio::io_service& io_service,smtp_server* server) : socket(io_service),server(server)
{
...
void session::start()
{
    ...
    std::string greeting_message = "220 smtp.example.com ESMTP MailD ready";
